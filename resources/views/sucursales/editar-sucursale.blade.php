    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="formulario">
                ...
                <form action="{{ route('update-sucursal',$sucursal->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    
                    <div class="form-group">
                        <label for="nombre">Nombre del Sucursal:</label>
                        <input type="text" class="form-control" id="nombre" name="nombre" value="{{$sucursal->name}}" required>
                    </div>
                    <div class="form-group">
                        <input type="text" class="form-control" id="latitude" name="latitude" value="{{$locations->latitude}}" hidden>
                    </div>
                    <div class="form-group">
                        <input type="text" class="form-control" id="longitude" name="longitude" value="{{$locations->longitude}}" hidden>
                    </div>
                    <div class="form-group">
                        <label for="descripcion">Descripción:</label>
                        <textarea class="form-control" id="descripcion" name="descripcion" rows="3">{{$sucursal->description}}</textarea>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <img id="foto_preview" class="preview-img" src="{{$sucursal->image}}" alt="Preview Image">
                        </div>
                    </div>
...
<script>
    ...
    const inputLongitude = document.querySelector('#longitude');
        ...
        var map = L.map('map').setView([inputLatitude.value,inputLongitude.value], 14);
            L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
            maxZoom: 20,
            attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'
        }).addTo(map);
        marker =L.marker([inputLatitude.value,inputLongitude.value], {draggable: true}).addTo(map);
